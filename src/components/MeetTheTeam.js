...
export default function MeetTheTeam() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-14 border-t border-gray-100">
      <h2 className="text-3xl font-bold text-center text-gray-900 mb-1">{title}</h2>
      {subtitle && (
        <p className="text-center text-gray-400 text-sm mb-12">{subtitle}</p>
      )}
          ...
          <div key={member.id} className="flex flex-col items-center text-center">
            {member.image ? (
              ...
              <img
                src={member.image}
                alt={member.name}
                className="w-32 h-32 rounded-full object-cover mb-4 ring-4 ring-lime-100 shadow-sm"
              />
            ) : (
              <div className="w-32 h-32 rounded-full bg-lime-50 ring-4 ring-lime-100 flex items-center justify-center mb-4 shadow-sm">
            ...
            <p className="text-lime-700 text-sm font-semibold mt-0.5 mb-3">{member.title}</p>
