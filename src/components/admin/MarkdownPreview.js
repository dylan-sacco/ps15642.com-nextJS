...
export default function MarkdownPreview({ body }) {
  ...
  const CustomStyle = {
    img: ({ node, src, alt, ...props }) => (
      <img
        src={src}
        alt={alt || ''}
        className="rounded-lg ml-4 w-[50%] float-right object-cover shadow-sm"
        loading="lazy"
        {...props}
      />
    ),
  ...
  //       src={src}
  //       alt={alt || ''}
  //       className="rounded-lg ml-4 w-[50%] float-right object-cover shadow-sm rotate-2 hover:rotate-0 transition-transform duration-300"
  //       loading="lazy"
  //       {...props}
  //     />
  //   ),
