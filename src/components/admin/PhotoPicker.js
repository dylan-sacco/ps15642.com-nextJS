...
export default function PhotoPicker({ textareaRef, onInsert }) {
  ...
  function insertPhoto(e, filename) {
    e.preventDefault();
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-2xl w-80 max-h-96 flex flex-col overflow-hidden">
          ...
          <div className="overflow-y-auto flex-1 p-2">
            ...
            {!loading && images.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-6">No photos in gallery.</p>
            )}
            {!loading && images.length > 0 && (
              <div className="grid grid-cols-3 gap-1.5">
                {images.map(img => (
                  <button
                    key={img.filename}
                    onMouseDown={e => insertPhoto(e, img.filename)}
                    className="aspect-square rounded overflow-hidden border border-transparent hover:border-lime-500 focus:outline-none focus:border-lime-500 transition-colors group"
                    title={img.filename}
                  >
